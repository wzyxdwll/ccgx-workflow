...
qualityPlan = buildQualityPlan(
  ...
  { phaseId, phaseType, quality, workdir, jobId },
  pluginAvailability,
...
主线读 `qualityPlan.waves[i].spawns[0]`（impl wave）的 `bashCommand` 字段直接
跑：

```bash
# 1. 把 phase prompt 写到 .context/jobs/<job-id>/prompt.txt（主线先 Write）
# 2. 用 Bash run_in_background=true 启动子进程：
Bash(claude -p "$(cat '.context/jobs/<job-id>/prompt.txt')" \
...
prompt.txt 的内容与 v4.0~v4.4 的 `Agent(subagent_type=...)` prompt 完全一致：
...
**禁用** `Agent(subagent_type="phase-runner")` —— 与本模板已弃用。仅在 PoC 旧
路径或非 autonomous 调用方仍可见，autonomous 主线**不得**回退到该路径（除非
P1f gate 失败显式降级，由 release notes 公示）。
...
**走 phase-runner 路径**（4.2 第 2 路，v4.5 P1a Bash 直调）：

- 主线 `Bash(run_in_background=true)` 启动 `claude -p --agent ccg/phase-runner ...` 子进程后**不阻塞**继续做别的；用 `Bash(wc -l .context/jobs/<id>/progress.jsonl)` / `Bash(tail -n 5 ...)` 轮询观察进度
