...
**模型路由委派给 phase-runner**：autonomous 主线不再硬编码 codex/gemini，phase-runner 根据 prompt 里的 `phase_type` 字段决定 spawn `codex:rescue` 还是 `gemini:rescue`。这修复了 v3.0 路由 bug（autonomous 绕过 `{{FRONTEND_PRIMARY}}/{{BACKEND_PRIMARY}}` 配置）。
...
- **4.4.a Critical phase challenger 编排（v4.1 Phase 16）**：
  ...
  |-----------|------------------------------|
  | `backend`  | `Agent(codex:rescue)` + `Agent(assumptions-analyzer)` |
  | `frontend` | `Agent(gemini:rescue)` + `Agent(nyquist-auditor)` |
  | `fullstack`| `Agent(codex:rescue)` + `Agent(gemini:rescue)` + `Agent(assumptions-analyzer)` + `Agent(nyquist-auditor)` |
  | `docs`/`generic` | `Agent(assumptions-analyzer)` 单兵 |

  **plugin 缺失降级**（acceptance d）：installer 检测不到 `codex:rescue` 或
  `gemini:rescue` 命令时，主线只 spawn specialist（CCG 自家 agent，必装），
