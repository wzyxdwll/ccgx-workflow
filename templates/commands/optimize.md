...
1. **优先 plugin spawn**（默认）：装了 `codex@openai-codex` + `gemini@google-gemini` plugin → 用 `Agent(subagent_type="codex:rescue")` + `Agent(subagent_type="gemini:rescue")` 并行，主线接 ≤200 token 摘要。
